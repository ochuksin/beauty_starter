...
function CAForm() {
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    // из-за асинхронной работы - это будет неправильно работать, но перенести в createNewAppointment в AppointmentService.ts
    //   ...prevData,
  ...
  const handleChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    // обновляем поле, которое в данный момент изменяется
    // prevData - предыдущий state
  ...
  return (
    <form className="caform" onSubmit={handleSubmit}>
      ...
      <input
        ...
        name="phone"
        id="phone"
        placeholder="+1 890 335 37 45"
        pattern="^\++[0-9]{1} [0-9]{3} [0-9]{3} [0-9]{2} [0-9]{2}"
        title="Format should be +1 890 335 37 45"
        required
